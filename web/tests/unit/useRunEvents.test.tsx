...
let replays: Map<string, () => Promise<unknown>>;
...
beforeEach(() => {
  ...
  vi.stubGlobal(
    ...
    vi.fn(async (url: string) => {
      ...
      const body = await (replays.get(runId) ?? (async () => []))();
...
describe("useRunEvents", () => {
  it("ends the run when the replay already holds a valid outcome, and closes the stream", async () => {
    replays.set(RUN_A, async () => [env(RUN_A, 1, "run.created", { request_text: "raise Pro" }), env(RUN_A, 2, "run.outcome", { outcome: "SUCCESS" })]);
  ...
  it("does not end the run on a run.outcome with an invalid outcome", async () => {
    const useRunEvents = await loadHook();
    const { result } = renderHook(() => useRunEvents(RUN_A));
    ...
    await waitFor(() => expect(result.current.view?.rejected).toEqual([{ seq: 1, type: "run.outcome" }]));
  ...
  it("merges a slow replay with a live event that arrived first, in sequence order", async () => {
    ...
    replays.set(RUN_A, () => new Promise((resolve) => (release = resolve)));
    const useRunEvents = await loadHook();
    const { result } = renderHook(() => useRunEvents(RUN_A));
    const source = FakeEventSource.instances[0];

    act(() => source.emit("ledger.completed", env(RUN_A, 2, "ledger.completed", { ledger_id: 7, entry_hash: "bb" })));
    await act(async () => release([env(RUN_A, 1, "ledger.pending", { ledger_id: 7, step_no: 1, app: "stripe" })]));
